...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, done) => done(null, `${Date.now()}-${Math.random().toString(16).slice(2)}${path.extname(file.originalname || '').toLowerCase().slice(0, 10)}`)
  }),
...
app.use(session({
  name: 'cipa.sid', store: new SqliteSessionStore(db),
...
function ensureAdmin() {
  ...
  if (!user) db.prepare('INSERT INTO users (username, password_hash) VALUES (?, ?)').run(username, bcrypt.hashSync(password, 12));
}
ensureAdmin();
try { console.log(`Backup diário: ${runDailyBackup(db, { projectRoot, dataDir })}`); } catch (error) { console.error('Aviso: não foi possível criar o backup diário.', error.message); }

const addAudit = (recordId, username, action, details = '') => db.prepare('INSERT INTO audit_logs (record_id, username, action, details) VALUES (?, ?, ?, ?)').run(recordId, username, action, details);

app.get('/', (req, res) => res.render('public-form', { types: TYPES, success: req.query.success === '1', error: null, values: {} }));
app.post('/enviar', submissionLimiter, upload.fields([{ name: 'photos', maxCount: 3 }, { name: 'camera', maxCount: 1 }]), (req, res) => {
  const uploadedFiles = Object.values(req.files || {}).flat();
  const values = {
    employee_name: clean(req.body.employee_name, 150),
    department: clean(req.body.department, 120), job_title: clean(req.body.job_title, 120),
    type: clean(req.body.type, 50), description: clean(req.body.description, 5000)
  };
  ...
  try {
    ...
    const result = db.prepare(`INSERT INTO records (employee_name, department, job_title, type, description)
      VALUES (?, ?, ?, ?, ?)`).run(values.employee_name, values.department, values.job_title, values.type, values.description);
    const addAttachment = db.prepare('INSERT INTO attachments (record_id, filename, original_name, mime_type) VALUES (?, ?, ?, ?)');
    uploadedFiles.forEach(file => addAttachment.run(result.lastInsertRowid, file.filename, clean(file.originalname, 255), file.mimetype));
    addAudit(result.lastInsertRowid, 'Formulário público', 'Registro criado', `${values.type} - ${values.department}`);
    db.exec('COMMIT');
...
function whereFor(filters) {
  const where = [], params = [];
  if (filters.from) { where.push("date(created_at, 'localtime') >= date(?)"); params.push(filters.from); }
  if (filters.to) { where.push("date(created_at, 'localtime') <= date(?)"); params.push(filters.to); }
  ...
  if (filters.q) { where.push('(employee_name LIKE ? OR description LIKE ?)'); params.push(...Array(2).fill(`%${filters.q}%`)); }
  return { sql: where.length ? ` WHERE ${where.join(' AND ')}` : '', params };
}

app.get('/admin', auth, (req, res) => {
  const filters = filtersFrom(req.query), clause = whereFor(filters);
  const records = db.prepare(`SELECT * FROM records${clause.sql} ORDER BY id DESC`).all(...clause.params);
  const statsRows = db.prepare('SELECT status, COUNT(*) total FROM records GROUP BY status').all();
  const counts = Object.fromEntries(statsRows.map(x => [x.status, x.total]));
  const departments = db.prepare("SELECT DISTINCT department FROM records WHERE department <> '' ORDER BY department").all().map(x => x.department);
  const critical = db.prepare("SELECT COUNT(*) total FROM records WHERE priority='Crítica' AND status NOT IN ('Resolvido','Arquivado')").get().total;
  const overdue = db.prepare("SELECT COUNT(*) total FROM records WHERE due_date IS NOT NULL AND due_date < date('now','localtime') AND status NOT IN ('Resolvido','Arquivado')").get().total;
  res.render('admin', { records, filters, counts, critical, overdue, total: db.prepare('SELECT COUNT(*) total FROM records').get().total, departments, types: TYPES, statuses: STATUSES, priorities: PRIORITIES, user: req.session.user });
});
app.get('/admin/registro/:id', auth, (req, res) => {
  ...
  const attachments = db.prepare('SELECT * FROM attachments WHERE record_id = ? ORDER BY id').all(record.id);
  const history = db.prepare('SELECT * FROM audit_logs WHERE record_id = ? ORDER BY id DESC').all(record.id);
  res.render('record', { record, attachments, history, statuses: STATUSES, priorities: PRIORITIES, saved: req.query.saved === '1' });
});
app.get('/admin/anexo/:id', auth, (req, res) => {
  ...
  res.type(attachment.mime_type).sendFile(path.join(uploadDir, attachment.filename));
});
app.post('/admin/registro/:id', auth, (req, res) => {
  ...
  db.prepare('UPDATE records SET status=?, priority=?, due_date=?, internal_notes=?, assigned_to=?, resolution=? WHERE id=?')
    .run(status, priority, dueDate, notes, assignedTo, resolution, id);
  const changes = [];
  ...
  if (changes.length) addAudit(id, req.session.user.username, 'Registro atualizado', changes.join(' | '));
  res.redirect(`/admin/registro/${id}?saved=1`);
});
app.post('/admin/registro/:id/arquivar', auth, (req, res) => {
  const id = Number(req.params.id);
  db.prepare("UPDATE records SET status='Arquivado' WHERE id=?").run(id);
  addAudit(id, req.session.user.username, 'Registro arquivado');
  res.redirect('/admin');
});
app.post('/admin/registro/:id/excluir', auth, (req, res) => {
  const id = Number(req.params.id);
  const files = db.prepare('SELECT filename FROM attachments WHERE record_id=?').all(id);
  addAudit(id, req.session.user.username, 'Registro excluído');
  db.prepare('DELETE FROM records WHERE id=?').run(id);
  files.forEach(file => fs.rmSync(path.join(uploadDir, file.filename), { force: true }));
  res.redirect('/admin');
});

function getReportRecords(query) {
  const filters = filtersFrom(query), clause = whereFor(filters);
  const records = db.prepare(`SELECT * FROM records${clause.sql} ORDER BY id DESC`).all(...clause.params);
  const attachmentQuery = db.prepare('SELECT * FROM attachments WHERE record_id = ? ORDER BY id');
  records.forEach(record => { record.attachments = attachmentQuery.all(record.id); });
  return { records, filters };
}

app.get('/admin/relatorio', auth, (req, res) => {
  const { records, filters } = getReportRecords(req.query);
  res.render('report', { records, filters, generatedAt: new Date() });
});

app.get('/admin/relatorio.pdf', auth, (req, res, next) => {
  try {
    const { records } = getReportRecords(req.query);
    ...
    records.forEach((record) => {
      ...
      const printableImages = record.attachments.filter(image => ['image/jpeg', 'image/png'].includes(image.mime_type) && fs.existsSync(path.join(uploadDir, image.filename))).slice(0, 4);
      if (printableImages.length) {
        ...
        printableImages.slice(0, 2).forEach((image, index) => {
          const x = startX + index * (imageWidth + imageGap);
          try { doc.image(path.join(uploadDir, image.filename), x, imageY, { fit: [imageWidth, imageHeight], align: 'center', valign: 'center' }); } catch {}
        });
...
app.get('/admin/exportar.csv', auth, (req, res) => {
  const clause = whereFor(filtersFrom(req.query));
  const rows = db.prepare(`SELECT * FROM records${clause.sql} ORDER BY id DESC`).all(...clause.params);
...
app.get('/admin/qrcode', auth, async (req, res, next) => {
  try {
    ...
    const wanUrl = clean(req.query.wanUrl, 300) || process.env.PUBLIC_URL || 'http://168.0.96.134:3000';
